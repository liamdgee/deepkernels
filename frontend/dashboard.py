...
app = dash.Dash(
    __name__,
    title="DeepKernels -- A quantitative risk engine",
...
X_TEXT = "Simulated Credit Applications"
Y_TEXT = "Increased Likelihood of Rejection (%)"
    ...
    <head>
        ...
        <style>
            ...
            .ios-tabs-container {
                ...
                padding: 3px !important; /* Uniform padding keeps the tab inside */
                height: 36px !important;
                box-sizing: border-box !important;
                border: none !important;
            }

            /* The Fix: Force Dash's injected wrapper to behave */
            .ios-tabs-container > div {
                display: flex !important;
                width: 100% !important;
            ...
            .ios-tab {
                ...
                /* 👇 THE SPACERS 👇 */
                padding: 0 16px !important;     /* Adds horizontal breathing room */
                min-width: 50px !important;     /* Forces them to not squish */
            ...
            .ios-tab--active {
                background-color: #ffffff !important;
                color: #0071e3 !important; 
                box-shadow: 0 1px 4px rgba(0, 0, 0, 0.12), 0 1px 1px rgba(0, 0, 0, 0.05) !important;
                border: none !important;
                border-top: none !important;
                border-bottom: none !important;
            }
            ...
            /* The wrapper around the info icon */
            ...
            /* Optional: The little triangle pointing down */
...
def Spacer(size=10):
    return html.Div(style={"height": f"{size}px"})


def create_label_with_tooltip(label_text, tooltip_text, label_style):
    return html.Div(
        [
            html.Label(label_text, style={**label_style, "marginBottom": "0px"}),
            html.Div(
                ...
                children=[
                    ...
                    html.Span(
                        tooltip_text, className="custom-tooltip-text"
                    ),  # The hidden text
                ],
            ),
        ],
...
def create_radio_label(text, tooltip_text):
    return html.Div(
        className="custom-tooltip-container",
        style={"display": "inline-flex", "alignItems": "center", "gap": "4px"},
        children=[
            html.Span(text),
            html.Span("ⓘ", style={"color": "#8b949e", "fontSize": "12px"}),
            html.Span(
                tooltip_text,
                className="custom-tooltip-text",
                style={"fontWeight": "normal"},
            ),  # Ensures tooltip text doesn't inherit button bolding
        ],
    )
    ...
    with httpx.Client() as client:
        try:
            response = client.post(
                f"{API_BASE_URL}/inference/simulate", json=payload, timeout=45.0
            )

            # If the API returned an error, grab the specific FastAPI 'detail' message
            if response.status_code != 200:
                error_payload = response.json()
                # Return the error cleanly packaged so Dash can read it
                return {
...
def generate_gp_paths(data: dict | None, num_scribbles=12) -> go.Figure:
    ...
    # --- THE FIX ---
    # The callback already handled the logic! We just pull the exact values here.
    # We default to 1.96 and the 95% label just in case the dict is missing them.
    k_sigma = float(data.get("sigma_scale", 1.96)) 
    sigma_label = data.get("sigma_label", "2σ (95%)")

    # Apply the multiplier to the API's standard deviation
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name=f"{sigma_label} Confidence Interval",
            showlegend=False,
            hoverinfo="skip",
        )
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=mean,
            mode="lines+markers",
            name="Rejection Likelihood Trajectory (μ)",
    ...
    fig.update_layout(
        **BESPOKE_LAYOUT,
        title={
            "text": f"<b>GP Posterior Manifold</b> <span style='color:#86868b; font-weight:normal'>({sigma_label} Scaling)</span>",
            "y": 0.95,
            "x": 0.5,
            "xanchor": "center",
            "yanchor": "top",
        ...
        updatemenus=[
            dict(
                ...
                buttons=[
                    dict(
                        label="Show Simulation Paths",
                        method="update",
                        args=[{"visible": show_all}],
                    ),
                    dict(
                        label="Hide Simulation Paths",
                        method="update",
                        args=[{"visible": hide_scribbles}],
                    ),
                ],
        ...
        children=[
            ...
            html.Div(
                ...
                children=[
                    html.Div(
                        [
                            html.Div("Key Loan Descriptors", style=SECTION_TITLE),
                            create_label_with_tooltip(
                                "Borrower Tenure",
                                "Number of months that loan applicant has worked at current company preceeding B2B loan application.",
                                LABEL_STYLE,
                            ),
                            Spacer(10),
                            dcc.Slider(
                            ...
                            create_label_with_tooltip(
                                ...
                                "Requested loan repayment period (months). This corresponds to model depth.",
                                LABEL_STYLE,
                            ),
                            Spacer(10),
                            dcc.Slider(
                    ...
                    html.Div(
                        [
                            html.Div("Proxies for Lender Bias", style=SECTION_TITLE),
                            Spacer(10),
                            create_label_with_tooltip(
                                "Location Risk",
                                "Quantifies empirical lender bias against applicants in US regions with lower average education levels and higher crime rates.",
                                LABEL_STYLE,
                            ),
                            Spacer(10),
                            dcc.Slider(
                            ...
                            create_label_with_tooltip(
                                "Socioeconomic Risk",
                                "Estimates lender-perceived default risk based on average income, wealth distribution, and public infrastructure quality within the applicant's ZIP code.",
                                LABEL_STYLE,
                            ),
                            Spacer(10),
                            dcc.Slider(
                            ...
                            Spacer(20),
                            create_label_with_tooltip(
                                "Algorithmic Bias Score (IAT)",
                                "Calculates the algorithm's Implicit Association Test (IAT) score, measuring latent associations between protected borrower demographics and likelihood of application rejection.",
                                LABEL_STYLE,
                            ),
                            Spacer(10),
                            dcc.Slider(
                    ...
                    html.Div(
                        ...
                        children=[
                            dcc.Checklist(
                                ...
                                options=[
                                    {"label": "Single Lender View", "value": "selected"}
                                ],
                            ...
                            html.Div(
                                [
                                    ...
                                    dcc.RadioItems(
                                        ...
                                        options=[
                                            {
                                                "label": create_radio_label(
                                                    "BANK",
                                                    "Traditional depository financial institutions subject to strict regulatory guidelines and oversight.",
                                                ),
                                                "value": "bank",
                                            },
                                            {
                                                "label": create_radio_label(
                                                    "FINTECH",
                                                    "Non-Depository financial institutions typically leveraging alternative data methods, model-driven application approval processes and automated underwriting.",
                                                ),
                                                "value": "fintech",
                                            },
                                            {
                                                "label": create_radio_label(
                                                    "CREDIT UNION",
                                                    "Not-for-profit financial cooperatives with community-focused objectives of providing cost-effective financial services to private members.",
                                                ),
                                                "value": "creditunion",
                                            },
                                            {
                                                "label": create_radio_label(
                                                    "CDFI",
                                                    "Community Development Financial Institutions built explicitly to serve low-income and disadvantaged communities.",
                                                ),
                                                "value": "cdfi",
                                            },
                                            {
                                                "label": create_radio_label(
                                                    "MDI",
                                                    "Minority Depository Institution. A federally insured bank or credit union predominantly owned or directed by minority individuals to serve disadvantaged communities.",
                                                ),
                                                "value": "mdi",
                                            },
                                            {
                                                "label": create_radio_label(
                                                    "MCA",
                                                    "Merchant Cash Advance / Factoring. Rapid, high-cost alternative B2B financing based on future receivables rather than traditional credit risk factors.",
                                                ),
                                                "value": "factoringccmca",
                                            },
                                        ],
                                        value="fintech",
                    ...
                    # 4. PRIMARY TRIGGER & MODAL LINK (Grouped as a Footer)
                    html.Div(
                        style={
                            ...
                            "display": "flex",
                            "flexDirection": "column",
                            "gap": "12px",  # Clean spacing between the two buttons
                        },
                        children=[
                            html.Button(
                            ...
                            # Now the "About" button lives inside the bottom-anchored container
                            html.Button(
                                "ⓘ About the Engine",
                                id="open-about-modal",
                                n_clicks=0,
                                style={
                                    ...
                                    "alignItems": "center",
                                    "gap": "5px",
                                    "justifyContent": "center",  # Centers the text/icon within the button width
                                },
                            ),
                        ],
                    ),
                                    ...
                                    dcc.Tabs(
                                        ...
                                        children=[
                                            dcc.Tab(
                                                ...
                                                value="1.0",
                                                className="ios-tab",
                                                selected_className="ios-tab--active",
                                            ),
                                            dcc.Tab(
                                                label="2σ",
                                                value="1.96",
                    ...
                    children=[
                        ...
                        html.Div(
                            children=[
                                ...
                                html.Div(
                                    "ALGORITHMIC BIAS DETECTION ENGINE",
                                    style={
                                        "fontSize": "10px",
                                        "color": "#8b949e",
                                        "letterSpacing": "2px",
                                    ...
                                    children=[
                                        ...
                                        html.P(
                                            [
                                                ...
                                                " engine is currently powering the Algorithmic Auditing Simulator on the main dashboard. "
                                                "It measures dynamic changes in the topology of the probability space with respect to each selectable lender type—hence ",
                                                html.A(
                                                    "topologicaldisparity.com",
                                                    href="https://topologicaldisparity.com",
                                                    target="_blank",
                                                    style={
                                                        "color": "#0071e3",
                                                        "textDecoration": "none",
                                        ...
                                        html.Div(
                                            ...
                                            children=[
                                                html.A(
                                                    "linkedin.com/in/liamdouglasgiles",
                                                    href="https://www.linkedin.com/in/liamdouglasgiles",
                                                    target="_blank",
                                                    style={
                                                        "color": "#3B82F6",
                                                        "textDecoration": "none",
                                                ...
                                                html.A(
                                                    "github.com/liamdgee/deepkernels",
                                                    href="https://github.com/liamdgee/deepkernels",
                                                    target="_blank",
                                                    style={
                                                        "color": "#3B82F6",
                                                        "textDecoration": "none",
...
def run_simulation(
    ...
    payload = {
        ...
        "animus_proxy": float(np.clip(animus, MINAN, MAXAN)),
        "isolation_proxy": float(np.clip(isolation, MINISO, MAXISO)),
        "iat_score": float(np.clip(iat, MINIAT, MAXIAT)),
        "horizon_steps": int(horizon_steps),
        "compare_all_lenders": is_compare,
    }
    # --- THE FIX: Clean Map for the UI Labels ---
    z_score = float(sigma_val) if sigma_val is not None else 1.96
    base_scale = 0.001
    k_sigma = round(z_score * base_scale, 3)

    # Map the precise z-score back to natural language for the graphs
    sigma_display_map = {"1.0": "1σ (68%)", "1.96": "2σ (95%)", "2.58": "3σ (99%)"}
    # This will output "2σ (95%) Confidence" on the graph instead of "1.96σ"
    clean_sigma_label = sigma_display_map.get(str(sigma_val), f"{z_score:g}σ")

    data = trigger_simulation(payload)

    if not data or "detail" in data:
        error_msg = data.get("detail", "Check Port 8000.") if data else "API Offline"
        return (
            go.Figure().update_layout(
                title="❌ API CONNECTION FAILED", **BESPOKE_LAYOUT
            ),
            error_msg,
        )
    ...
    if is_compare and isinstance(data, dict):
        ...
        for lender_name, metrics in data.items():
            ...
            status_msg = (
                f"Real-time Approval Bias & Uncertainty Bands for {display_name}"
            )
        ...
        fig.update_layout(
            **BESPOKE_LAYOUT,
            title={
                "text": (
                    f"<b>Projected Loan Rejection Rates</b>"
                    if is_compare
                    else f"<b>Projected Exposure: {lender.upper()}</b>"
                ),
                "y": 0.95,
                "x": 0.5,
                "xanchor": "center",
        ...
        gp_manifold_data = {
            ...
            "std": single_lender_data.get(
                "absolute_std", []
            ),  # Use absolute_std for the manifold wash

...
def test_amortised_dirichlet_forward_and_backward():
    ...
    mock_vae_out = {
        ...
        'factor_alpha': torch.randn(batch_size, k_atoms, 3), # Assuming rank 3 for lowrank norm
    ...
    assert isinstance(out, dict), "Model must return a dictionary"
    ...
    assert model.kernel_network.linear.weight.grad is not None, "Gradients failed to reach the internal KernelNetwork"

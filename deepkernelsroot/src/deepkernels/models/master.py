...
class DeepGaussianProcess(gpytorch.models.ApproximateGP):
    def __init__(self, 
        ...
        self.encoder = encoder if encoder else Encoder(config=VAEConfig())
        self.dirichlet = dirichlet if dirichlet else AmortisedDirichlet(config=HDPConfig())
        ...
        #-mean module-#
        self.mean_module = gpytorch.means.ConstantMean(input_size=self.latent_dim)
        
        #-Cov module (stateless)-#
        self.covar_module = gpytorch.kernels.ScaleKernel(
            DeepKernel(num_experts=self.k_experts, batch_mode=True), 
    ...
    def forward(self, x):
        ...
        pi, beta, means, bw = self.dirichlet(z, rff_kernel=False)

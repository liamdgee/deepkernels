...
class BaseGenerativeModel(gpytorch.Module):
    ...
    def multivariate_projection(self, mu, factor, diag, jitter=1e-6):
        ...
        mvn = dist.LowRankMultivariateNormal(loc=mu, cov_factor=factor, cov_diag=diag)
    ...
    def inverse_softplus(self, target_value, min=3e-7):
        ...
        return torch.where(
            safe_target > 20.0, safe_target, torch.log(torch.expm1(safe_target))
        )
    ...
    def get_device(
        self, device_request: Union[str, torch.device, None] = None
    ) -> torch.device:
        """
        ...
        if device_request is not None:
            ...
            if device.type == "mps" and not torch.backends.mps.is_available():
                logging.warning(
                    f"MPS (Apple Silicon) requested but unavailable. Falling back to CPU."
                )
                return torch.device("cpu")
            return device
    ...
    def lowrankmultivariatenorm(self, mu, factor, diag):
        mvn = torch.distributions.LowRankMultivariateNormal(
            loc=mu, cov_factor=factor, cov_diag=diag
        )
        logits = mvn.rsample()
        return logits

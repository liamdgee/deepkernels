...
# -key imports-#
try:
    from deepsecretsroot.deepkernels.models.model import StateSpaceKernelProcess
except ImportError:
...
WEIGHTS_PATH = Path(
    os.getenv("MODEL_WEIGHTS_PATH", ARTIFACTS_DIR / "princess_weights.pth")
)
...
# ==========================================
# ==========================================
# TORCH & KEOPS CONFIGURATION (Optimized for A100)
# ==========================================
...
# --- SPEED TWEAK 1: Downgrade to float32 ---
# A100 Tensor Cores are 10x+ faster on float32/TF32 than float64
pykeops.config.precision = "float32"
torch.set_default_dtype(torch.float32)

# --- SPEED TWEAK 2: Enable TF32 (The A100 Secret Sauce) ---
# This allows the GPU to use specialized hardware for matrix math
torch.backends.cuda.matmul.allow_tf32 = True
torch.backends.cudnn.allow_tf32 = True

# Threading remains at 1 to prevent container lock-up
...
class SimulationInput(BaseModel):
    ...
    lender_type: Literal[
        "fintech", "bank", "cdfi", "creditunion", "mdi", "factoringccmca"
    ] = "fintech"
    ...
    horizon_steps: int = Field(36, ge=12, le=64)
    ...
    try:
        ...
        if not hasattr(orch, "feature_transformer"):
            old_name = next(
                (
                    name
                    for name in ["transformer", "feature_eng", "novelty"]
            ...
            if old_name:
                logger.info(
                    f"🩹 Patching Orchestrator: Aliasing '{old_name}' -> 'feature_transformer'"
                )
                orch.feature_transformer = getattr(orch, old_name)
            else:
                logger.error(
                    "❌ ERROR: Could not find any transformer inside the pickle!"
                )
        ...
        schema_categories = set(
            SimulationInput.model_fields["lender_type"].annotation.__args__
        )
        if schema_categories - model_categories:
            logger.warning(
                f"⚠️ WARNING: Schema allows lenders not in training data: {schema_categories - model_categories}"
            )
        input_dim = (
            len(orch.config.feature.num_cols) + len(orch.config.feature.cat_cols) + 1
        )

        # 1. Initialize the model on CPU first (to be safe)
        model = StateSpaceKernelProcess(
            input_dim=input_dim, n_data=87636.0, device=torch.device("cpu")
        )
        ...
        # 1. Purify the dictionary BEFORE loading
        clean_state_dict = {
        ...
        # 2. Standard cast for parameters and buffers
        model = model.float()

        # 3. THE GHOST HUNTER: Forcibly downcast GPyTorch's hidden training data
        if hasattr(model, "gp"):
            if hasattr(model.gp, "train_inputs") and model.gp.train_inputs is not None:
                model.gp.train_inputs = tuple(
                    t.float() if torch.is_floating_point(t) else t
                    for t in model.gp.train_inputs
                )
            if (
                hasattr(model.gp, "train_targets")
                and model.gp.train_targets is not None
            ):
                model.gp.train_targets = model.gp.train_targets.float()

        model.gp.likelihood.noise_covar.register_constraint(
            "raw_noise", gpytorch.constraints.Interval(1e-5, 0.005)
        )
        model.gp.covar_module.register_constraint(
            "raw_outputscale", gpytorch.constraints.Interval(0.01, 0.45)
        )
        model.gp.covar_module.register_constraint(
            "raw_inv_bandwidth", gpytorch.constraints.Interval(0.01, 0.38)
        )
        logger.info(f"✅ Salvaged {len(clean_state_dict)} keys to CPU model.")
        try:
        ...
        if hasattr(model, "gp") and hasattr(model.gp, "likelihood"):
            model.gp.likelihood.eval()

        state["model"] = model
        state["device"] = device
        logger.info(f"✅ SleepyPrincessv1.0 Ready on {device}.")
        logger.info("🔥 Warming up GPU kernels (JIT Compilation)...")
        try:
            with torch.no_grad():
                dummy_x = torch.zeros(1, 32, input_dim, device=device)
                _ = model.generate_trajectory(dummy_x, horizon=32, device=device)
            logger.info("✅ GPU Warmup Complete. Engine is fully primed.")
        except Exception as e:
            logger.warning(f"⚠️ GPU Warmup skipped/failed: {e}")
...
async def efficiency_check(run_id: str):
    return {"gpu_util": 85.2, "latency_ms": 120}
...
def smooth_data(data: list, std_history: list = None) -> list:
    ...
    if std_history:
        ...
        beta = np.clip(base_beta + (avg_uncertainty * 0.11), 0.72, 0.92)
    else:
        beta = base_beta
...
def smooth_vectorized(data, std_data, b_beta=0.83):
    ...
    beta = np.clip(b_beta + (avg_u * 0.1), 0.72, 0.92)
    ...
    for t in range(1, data.shape[-1]):
        ...
        v_smoothed[..., t] = (v_smoothed[..., t - 1] * beta) + (
            data[..., t] * (1 - beta)
        )
...
async def run_simulation(payload: SimulationInput):
    ...
    if not payload.compare_all_lenders and payload.lender_type not in lender_pool:
        raise HTTPException(
            status_code=400, detail=f"Invalid lender: {payload.lender_type}"
        )

    active_lenders = (
        lender_pool if payload.compare_all_lenders else [payload.lender_type]
    )
    ...
    try:
        ...
        for l_type in active_lenders:
            raw = {
                ...
                **{
                    col: [1 if col == l_type else 0]
                    for col in orch.config.feature.cat_cols
                },
            }
        ...
        transformed = orch.transform_inference_input(full_df).to(device).float()
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var(
            ...
            time_tensor = torch.zeros(
                transformed.size(0), 1, dtype=torch.float32, device=device
            )
            transformed_30 = torch.cat([time_tensor, transformed], dim=1)
            x_t = transformed_30.view(num_samples, 1, -1)

            # Generate Trajectory
            mu_tensor, var_tensor = model.generate_trajectory(
                x_t, horizon=horizon, device=device
            )
        ...
        # Optional: Force PyTorch to release the memory back to the OS immediately
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

        # ==========================================
        # 5. CPU POST-PROCESSING (Your NumPy logic is perfect here)
        # ==========================================
        ...
        samples_np = mu_np[:, np.newaxis, :] + (
            epsilon_smooth * std_np[:, np.newaxis, :] * DAMPING
        )

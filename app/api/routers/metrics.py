...
from deepsecretsroot.app.core.database import get_db
from deepsecretsroot.app.services.telemetry import (fetch_calibration_history,
                                    fetch_dirichlet_stats, fetch_gp_paths,
                                    fetch_gradient_data, fetch_hw_telemetry,
                                    fetch_model_pulse, fetch_money_graph_data,
                                    fetch_nlpd_stats, get_batch_latency_stats)

# -router-#
router = APIRouter(prefix="/v1/metrics", tags=["Real-time Telemetry"])
...
async def dirichlet_convergence_endpoint(
    run_id: str, db: AsyncConnection = Depends(get_db)
):
    try:

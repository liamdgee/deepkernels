...
async def fetch_money_graph_data(
    run_id: str, db: AsyncConnection, sample_size: int = 200
) -> dict:
...
async def fetch_model_pulse(run_id: str, db: AsyncConnection) -> dict:
    ...
    score = 100
    if latest.get("val_ece", 0) > 0.05:
        score -= 20
    if latest.get("val_nlpd", 0) > 2.0:
        score -= 15
    if latest.get("gp_warmup_loss_total", 0) > 200:
        score -= 10
...
async def fetch_gradient_data(run_id: str, db: AsyncConnection) -> dict:
    ...
    query = text(
        "SELECT key, value FROM latest_metrics WHERE run_uuid = :run_id AND key LIKE 'grads/%'"
    )
    result = await db.execute(query, {"run_id": run_id})

    # Strip the "grads/" prefix and "_norm" suffix right here in the backend
    gradients = {
        row[0].split("/")[-1].replace("_norm", ""): row[1] for row in result.fetchall()
    }
...
async def fetch_calibration_history(run_id: str, db: AsyncConnection) -> dict:
    ...
    nlpd_history = [
        {"step": r.step, "val": r.value} for r in rows if r.key == "val_nlpd"
    ]

    return {"ece": ece_history, "nlpd": nlpd_history}
...
async def fetch_hw_telemetry(run_id: str, db: AsyncConnection) -> dict:
    ...
    query = text(
        "SELECT key, value FROM latest_metrics WHERE run_uuid = :run_id AND key LIKE 'system/%'"
    )
    result = await db.execute(query, {"run_id": run_id})
    ...
    return {
        ...
        "bottleneck": (
            "NVCC_COMPILER"
            if is_compiling
            else ("CPU_IO_BOUND" if gpu_util < 70 else "NONE")
        ),
        "recommendation": (
            "Increase num_workers" if not is_compiling and gpu_util < 70 else "Optimal"
        ),
    }
...
async def fetch_dirichlet_stats(run_id: str, db: AsyncConnection) -> dict:
    ...
    return {
        ...
        "gating_mode": (
            ...
            else "High-Local-Specialisation"
        ),
...
async def fetch_nlpd_stats(run_id: str, db: AsyncConnection) -> dict:
    ...
    if not row:
        return {
            ...
            "msg": "NLPD is usually logged at the end of an epoch.",
        }
